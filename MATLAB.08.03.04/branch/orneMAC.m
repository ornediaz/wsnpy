...
round = ones(1,N); % Packet from me that my parent expects
...
while ~isempty(evntL)
	...
	for bb = 1: N
		if length(chld{bb}) == 1 && strcmp(st{bb},'Wt4D') && ~prntRdy(chld{bb})
	...
	%prntRdy(12)
% 	if 	verbose && ~isempty(BOut{1}) && ~isempty(BOut{1}.Nr)
% 		disp('Hey')
% 	end 
	switch e.type
		case 'endPktRx' %{{{
...
%% NESTED FUNCTIONS 
	...
	function ackRx(x,Nr,forceReady)
	round(x) = Nr + 1;
	if length(BOut{x}) ~= 1

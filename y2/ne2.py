...
class PhyNet(Tree):
    ...
    def npakno(z, i, compf):
        ...
        n = int(np.ceil((z.n_descendants[i] + 1.0) / (1.0 + z.compf)))
        return n
    def npakto(z,compf):
        """Return the total number of generated packets."""
        n = sum(net.npakno(i,compf) for i in xrange(z.c) if z.f[i] >= 0)
        return n
    def bf_schedule(z, hops=2, compf=9999):
        ...
        while q:
            ...
            if hops == INF:
                ...
                s = k_neigh(node_set=z.f[x], hops=hops, tx_l=z.tx_l)
                # 2) Nodes whose parents can interfere with me
                s1 = k_neigh(node_set=x, hops=hops, tx_l=z.tx_l) 
                for w in s1:
                    s.update(z.chlr(w)) # Their parents
                # Start testing the next color to the one of the parent. 
                usedColors = set(tmp[x])
                for y in s:
                    usedColors.update(tmp[y])
                slots_parent = tmp[z.f[x]]
                if slots_parent:
        ...
        fin = [[max_slot_number + 1 - clr for clr in tmp[x]] for x xrange(c)]
        return fin
